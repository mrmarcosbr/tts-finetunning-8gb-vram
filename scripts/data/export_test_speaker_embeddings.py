...
Exemplo:
  python scripts/data/export_test_speaker_embeddings.py --config configs/config_train.yaml --dataset lapsbm_speecht5 --device cuda
"""
...
from tts.application.training.pipeline import extract_speaker_id
from tts.core.config import load_config
from tts.core.config_profile import resolve_training_profile
from tts.core.data_paths import repository_dataset_path
from tts.core.hardware import device_for_speechbrain


def load_all_items(full_cfg: Dict, ds_cfg: Dict) -> Tuple[List[Dict], Dataset]:
    local_ds_path = repository_dataset_path(full_cfg.get("settings") or {}, ds_cfg["dataset_id"])
...
def main() -> int:
    ...
    parser.add_argument("--profile", type=str, default=None, help="Perfil unificado no YAML")
    parser.add_argument("--dataset", type=str, default=None, help="Alias legado de --profile")
    parser.add_argument(
        "--output_dir",
        type=str,
    ...
    speaker_model = EncoderClassifier.from_hparams(
        source=encoder_id,
        run_opts={"device": device_for_speechbrain(dev)},
    )
