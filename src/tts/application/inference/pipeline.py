...
from tts.core.config import load_config
from tts.core.config_profile import merge_dataset_model_overrides, resolve_training_profile
from tts.core.data_paths import (
    exports_audio_root,
    exports_hf_test_split_path,
    repository_dataset_path,
    resolve_data_settings,
    speechbrain_cache_dir,
)
from tts.core.hardware import detect_hardware_profile, device_for_speechbrain
from tts.core.speakers import extract_speaker_id
...
from tts.infrastructure.audio.trained_post_eq import apply_trained_output_waveform_eq, inference_dc_and_highpass

_REPO_ROOT = Path(__file__).resolve().parents[4]
DEFAULT_CONFIG_INFERENCE_YAML = _REPO_ROOT / "configs" / "config_inference.yaml"
...
def exported_reference_wav_fallback_path(
    ...
    """WAV em data/exports/<profile>/audio/<speaker>/<stem>.wav (reconstruct_and_export)."""
    stem = reference_wav_stem_for_export(item)
    if not stem:
        return None
    settings = full_cfg.get("settings") or {}
    audio_root = os.path.abspath(exports_audio_root(settings, dataset_profile))
...
def resolve_reference_waveform_for_inference(
    ...
    *,
    ...
    """Áudio de referência do locutor: Arrow/HF; se vazio, WAV em data/exports/.../audio."""
    ...
    *,
    ...
    if pk < 1e-6:
        print(
            ...
            "verifica paths no split exportado ou ficheiros em data/exports/.../audio/."
        )
    else:
...
def load_inference_test_candidates_exported(full_cfg: Dict, dataset_name: str) -> List[InferenceCandidate]:
    settings = full_cfg.get("settings") or {}
    exported_path = exports_hf_test_split_path(settings, dataset_name)
...
def load_inference_candidates_from_stream(
    ...
    *,
    ...
    local_ds_path = repository_dataset_path(full_cfg.get("settings") or {}, dataset_id)
...
def main():
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "Omisso: usa configs/config_inference.yaml junto à raiz do repo, se existir. "
    ...
    parser.add_argument("--profile", type=str, default=None, help="Perfil unificado no YAML (ex.: lapsbm_speecht5)")
    parser.add_argument("--dataset", type=str, default=None, help="Alias legado de --profile (mesmo valor)")
    ...
    if getattr(args, "only_f0_rmse_dir", None):
        ...
        if not cf0 and not cwer:
            ...
            sys.exit(1)
        from tts.infrastructure.metrics.f0_infer_metrics import (
    ...
    try:
        resolved = resolve_training_profile(
            ...
            detect_hardware=detect_hardware_profile,
        )
    ...
    hw_name = args.profile or (
        profile_name if full_cfg.get("profiles") else detect_hardware_profile()
    )
    ...
    if model_type == "speecht5":
        ...
        sb_savedir = speechbrain_cache_dir(full_cfg.get("settings"), spk_enc_id)
        ...
        if getattr(args, "speecht5_zero_speaker_embedding", False):
            ...
            speecht5_speaker_model = EncoderClassifier.from_hparams(
                source=spk_enc_id,
                run_opts={"device": device_for_speechbrain(device)},
                savedir=sb_savedir,
        ...
        if infer_all_fastspeech2:
            ...
            if not test_part:
                print(
                    ...
                    "Confirma export `data/exports/.../hf_test_split` ou cache HF."
                )
                sys.exit(1)
        ...
        if has_ds_train:
            ...
            if not train_part:
                print(
                    ...
                    "Garante data/repository/lapsbm/ (save_to_disk) ou espelho em data/raw/."
                )
                sys.exit(1)
    ...
    def _wav_report(path: str, audio_arr, sr: int) -> None:
        ...
        if model_type == "speecht5" and n > 64 and pk < 1e-5:
            print(
                ...
                "(📊 acima) e export em data/exports/.../audio/."
            )
        ...
        for triple_fname_tag, triple_pass_kind in triple_emb_passes:
                    ...
                    if exported_npy_this_row:
                        ...
                        if emb_how == "x-vector_do_WAV_referência":
                            ...
                            print(
                                ...
                                "`data/exports/...` ou `--speecht5_zero_speaker_embedding`."
                            )
                        mix_mode = (
                            getattr(args, "speecht5_speaker_emb_mix_mode", "toward_neutral")
                            or "toward_neutral"
            ...
            if model_type == "fastspeech2":
                ...
                try:
                    ...
                    if getattr(args, "noise_reduce", False):
                        try:
                            from tts.infrastructure.audio.inference_noise_reduce import apply_noise_reduce_to_wav_file
                    ...
                    if effective_cand is not None and model_type == "speecht5" and effective_cand.dataset_item is not None:
                        ...
                        if ya is not None and ya.size > 0:
                            ...
                            print(
                                ...
                                f"data/exports/{dataset_name}/audio/<locutor>/)."
                            )
                except Exception as e:
