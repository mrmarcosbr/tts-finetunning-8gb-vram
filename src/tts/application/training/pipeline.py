...
from tts.core.data_paths import repository_dataset_path, training_cache_dir
from tts.core.hardware import detect_hardware_profile, device_for_speechbrain
from tts.core.speakers import extract_speaker_id as _extract_speaker_id
...
from tts.infrastructure.audio.speecht5_mel_utils import waveform_to_speecht5_label_tensor


def extract_speaker_id(item):
    return _extract_speaker_id(item, missing="default")
...
def apply_noise_reduce_to_dataset_audio(
    ...
    *,
    ...
    """
    from tts.infrastructure.audio.inference_noise_reduce import auto_pick_stationary_for_clip, noise_reduce_waveform
...
def main():
    ...
    parser.add_argument("--config", type=str, default="configs/config_train.yaml", help="Caminho do config_train.yaml")
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "(antes do cache em data/cache). O split de teste com NR é gravado em …/data/cache/…/test. "
    ...
    hw_profile = profile_name if full_cfg.get("profiles") else (args.profile or detect_hardware_profile(full_cfg))
    ...
    out_dir = os.path.join(hw_cfg['output_dir'], f"{model_type}-{profile_name}-{timestamp}")
    os.makedirs(out_dir, exist_ok=True)

    print(f"🚀 Iniciando Pipeline para: {model_type.upper()}")
    print(f"📂 Perfil: {profile_name} | Hardware: {hw_profile.upper()}")
    ...
    local_ds_path = repository_dataset_path(settings, ds_cfg["dataset_id"])
    ...
    if 'speaker_encoder_id' in model_cfg:
        ...
        speaker_model = EncoderClassifier.from_hparams(
            source=model_cfg['speaker_encoder_id'], run_opts={"device": device_for_speechbrain(device)}
        )
    ...
    cache_base_dir = training_cache_dir(
        settings,
        f"{model_type}_{profile_name}_{target_sr}hz{_nr_cache_tag}{_nr_gate_cache}{_fs2_cache_suffix}{_speecht5_mel_cache}{_spk_pool_cache}{_speecht5_cache_log10}",
    )
